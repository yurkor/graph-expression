                ...
                public Match nextInner() {
                    while (edgeIt.hasNext()) {
                        ...
                        if (matchP.evaluate(cur)) {
                            return new LeafMatch(context, node, context.getEndNode(cur), Collections.singletonList(cur));
                        }
                    }
                    return null;
                }
            ...
            return new MatchResult() {
                ...
                private Match greedyNextInner() {
                    while (!states.isEmpty()) {
                        ...
                        if (s.tail()) {
                            Node n = s.getEndNode();
                            states.add(new State(n, notZero(s.min - 1), notZero(s.max - 1), innerMatcher.find(n, context)));
                            continue;
                        }
