...
public class GraphExpChunker extends GraphRegExpMatchers implements Chunker {
    ...
    public GraphExpChunker(String rootCategory, Matcher matcher, String... strs) {
        ...
        while (!heads.isEmpty()) {
            ...
            if (m instanceof PredicateMatcherWithCats) {
                workOnCategories.addAll(((PredicateMatcherWithCats) m).categories);
            }
        }
    }
    ...
    public List<Node> toNode(Collection<Chunk> annsSet) {
        class NodeMap {
            ...
            public void putAnnot(Chunk ann) {
...
//				for (int i = 1; i < nodesList.size(); i++) {
    ...
    public static String toString(List<Node> node) {
        ...
        for (int i = 0, nodeSize = node.size(); i < nodeSize; i++) {
            ...
            for (Edge e : CONTEXT.getEdges(n)) {
                ...
                sb.append(ee.getAnn().type + "[PLACETEXTHERE").append("] n").append(ind).append(", ");
            }
    ...
    public Collection<Chunk> chunk(TextWithChunks chunkText) {
        ...
        List<Match> results = (matchFromAllPosition || matchAll) ? findFromAllPosition(matcher, node) : findAllLongest(matcher, node);
        for (Match m : results) {
    ...
    static Map<String, List<Chunk>> toGroups(Match m) {
        List<Chunk> chunks = Lists.newArrayList();
        Map<String, List<Chunk>> groups = Maps.newHashMap();
        groups.put(ROOT, toList(m.getList()));
        for (MarkedMatch mm : getAllMarked(m)) {
            groups.put(mm.markMatcher.groupName, toList(mm.getList()));
        }
        return groups;
    }
...
//                    return false;
//                }
//                InnerGraphMatcher.InsideContext context = new InnerGraphMatcher.InsideContext(CONTEXT, (PositionNode) endNode);
...
//    }
    ...
    public static Matcher matchPredicate(final org.apache.commons.collections15.Predicate<? extends Chunk> predicate, final String... category) {
...
//    }
    ...
    public static GraphRegExpExt.WeightedMatcher weighted(final Matcher m, final Transformer<Map<String, List<Chunk>>, Double> scorer) {
        return new GraphRegExpExt.WeightedMatcher(new Transformer<Match, Double>() {
            public Double transform(Match match) {
                ...
                return scorer.transform(toGroups(match));
            }
        }, m);
    }

    public static Transformer<Map<String, List<Chunk>>, Double> withGroupScorer(final String groupName, final double has, final double hasNot) {
        return new Transformer<Map<String, List<Chunk>>, Double>() {
            public Double transform(Map<String, List<Chunk>> stringListMap) {
                return stringListMap.get(groupName) != null ? has : hasNot;
            }
        };
    }

    private static class PredicateMatcherWithCats extends PredicateMatcher {
